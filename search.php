    ...
    { 
        if (strlen($query) < 1) {
            ...
            $q = "SELECT * FROM info WHERE ID LIKE '%$query%'
                  OR Name LIKE '%$query%' OR Fname LIKE '%$query%' OR Result LIKE '%$query%'";
            ...
            if (mysql_affected_rows() > 0) { 
                ...
                do {
                    ...
                    $text .= '<p><a> Результат поиска по запросу : '.$row1['ID'].'.'.'&nbsp'.$row['Name'].'&nbsp'.$row['Fname'].'&nbsp'.'-  '.$row['Result'].'.'.'</a></p>';
